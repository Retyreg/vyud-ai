...
async def ensure_user_credits(telegram_id: int, username: str = None, utm_source: str = 'organic'):
    try:
        ...
        if response.data:
            ...
            supabase.table('users_credits').insert({
                ...
                'total_generations': 0
                # Можем добавить поле 'utm_source': utm_source если создашь колонку в БД
            }).execute()
            
            # Логируем в отдельную таблицу аналитики (если есть)
            try:
                supabase.table('analytics_registrations').insert({
                    'telegram_id': telegram_id,
            ...
            print(f"✅ Новый пользователь {telegram_id} (@{username}) из {utm_source} получил {WELCOME_CREDITS} кредитов")
...
async def cmd_start(message: Message):
    ...
    # Извлекаем UTM-метку из deep link (если есть)
    # Формат: /start utm_source_campaign
    args = message.text.split()
    utm_source = args[1] if len(args) > 1 else 'organic'
    
    # Логируем источник трафика
    print(f"📊 Новый пользователь {telegram_id} из источника: {utm_source}")
    
    # Получаем/создаём пользователя
    credits = await ensure_user_credits(telegram_id, username, utm_source)
    
    if credits == WELCOME_CREDITS:
        welcome_text = (

...
try:
    ...
    from auth import save_quiz, get_user_quizzes
...
MAX_FILE_SIZE_MB = 20  # Telegram ограничение для ботов

# --- МЕНЮ ---
async def set_main_menu(bot: Bot):
    await bot.set_my_commands([
        BotCommand(command='/start', description='🚀 Начать'),
        BotCommand(command='/profile', description='⚡️ Мой профиль'),
...
def get_user_email(message: Message) -> str:
    """Генерирует email из Telegram username"""
    ...
    if test_id:
        buttons.append([
            InlineKeyboardButton(
                text="🌐 Открыть тест в браузере",
                url=f"{WEB_APP_URL}/?test={test_id}"
            )
        ])
    
    buttons.append([
        InlineKeyboardButton(
            text="💻 Веб-версия VYUD",
            url=WEB_APP_URL
        )
    ])
...
async def process_media_file(message: Message, file_id: str, file_name: str, is_audio: bool = False):
    """Универсальная обработка медиа (аудио/видео)"""
    ...
    try:
        ...
        if file_size_mb > MAX_FILE_SIZE_MB:
            await message.answer(
                ...
                f"Загрузите файл через веб-версию.",
                parse_mode="HTML",
                reply_markup=create_web_keyboard()
            )
            return
        ...
        if not quiz_data or not quiz_data.questions:
            await message.answer(
                "❌ <b>Не удалось создать вопросы</b>\n\n"
                "Возможно, текст слишком короткий или не содержит обучающей информации.",
                parse_mode="HTML"
            )
            return

        # 4. Сохранение в БД
        test_title = f"Тест из {'аудио' if is_audio else 'видео'} ({file_name[:30]})"
        questions_json = [
            {
                "scenario": q.scenario,
                "options": q.options,
        ...
        test_id = save_quiz(user_email, test_title, questions_json, "")
        ...
        await message.answer(
            ...
            f"⚡️ Баланс: {new_balance} кредитов\n\n"
            f"👇 <b>Проверь себя!</b>",
            parse_mode="HTML",
            reply_markup=create_web_keyboard(test_id)
        )
        ...
        for i, q in enumerate(quiz_data.questions, 1):
            try:
                ...
                logging.error(f"Poll send error: {e}")

    except Exception as e:
        logging.error(f"Media processing error: {e}")
        await message.answer(
            f"❌ <b>Ошибка обработки:</b> {str(e)[:100]}",
            parse_mode="HTML"
        )
...
async def process_document_file(message: Message, file_id: str, file_name: str):
    """Обработка документов (PDF, DOCX, PPTX)"""
    user_email = get_user_email(message)
    ...
    try:
        ...
        # 4. Сохранение
        test_title = f"Тест: {file_name[:50]}"
        questions_json = [
            {
                "scenario": q.scenario,
                "options": q.options,
...
async def cmd_start(message: Message):
    ...
    await message.answer(
        ...
        f"📚 Превращаю файлы в интерактивные викторины!\n\n"
        f"<b>Что я умею:</b>\n"
        f"• 🎤 Кружочки и аудио\n"
        f"• 🎥 Видео\n"
        f"• 📄 PDF, DOCX, PPTX\n\n"
        f"⚡️ Баланс: <b>{credits}</b> кредитов\n\n"
...
async def cmd_profile(message: Message):
    ...
    await message.answer(
        ...
        f"💳 Пополнить баланс можно на сайте:",
        parse_mode="HTML",
        reply_markup=create_web_keyboard()
    )
...
async def cmd_mytests(message: Message):
    user_email = get_user_email(message)
    tests = get_user_quizzes(user_email)
    
    if not tests:
        await message.answer(
            "📚 <b>У вас пока нет тестов</b>\n\n"
            "Отправьте файл для создания первого теста!",
            parse_mode="HTML"
        )
        return
    
    text = f"📚 <b>Ваши тесты ({len(tests)}):</b>\n\n"
    for i, test in enumerate(tests[:10], 1):
        text += f"{i}. {test.get('title', 'Без названия')}\n"
    
    text += f"\n💻 Управляйте тестами на сайте:"
...
async def cmd_help(message: Message):
    await message.answer(
        ...
        "1. Отправьте файл (аудио, видео, документ)\n"
        "2. Дождитесь создания викторины\n"
        "3. Пройдите тест прямо в Telegram\n\n"
        ...
        "<b>Ограничения:</b>\n"
        "• Макс. размер: 50 MB\n"
        "• 1 кредит = 1 викторина\n\n"
        "💻 Больше возможностей на сайте:",
        parse_mode="HTML",
        reply_markup=create_web_keyboard()
    )
...
async def handle_audio(message: Message):
    ...
    await process_media_file(
        ...
        message.audio.file_name or f"audio_{message.audio.file_unique_id}.mp3",
        is_audio=True
    )
...
async def handle_document(message: Message):
    ...
    if not any(file_name.endswith(ext) for ext in supported_exts):
        await message.answer(
...
async def handle_other(message: Message):
    ...
    await message.answer(
        "🤔 <b>Не понимаю</b>\n\n"
        "Отправьте файл для создания викторины!\n\n"
